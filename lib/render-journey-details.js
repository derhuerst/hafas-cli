	...
	const renderLeg = (rows, l, i, legs) => {
		if (i === 0) {
			const row = [
				node,
				// todo: cancelled?
				chalk.cyan(renderTime(l.plannedDeparture, l.departureDelay)),
		...
		if (opt.showTripIds) row.push(chalk.gray(l.tripId || '?'))
		const nL = legs[i + 1]
		if (i > 0 && nL) {
		...
		rows.push([
			node,
			chalk.cyan(renderTime(l.plannedArrival, l.arrivalDelay)),
