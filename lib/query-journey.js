...
const createQueryJourney = (hafas, opt) => {
	...
	const renderJourneySummary = (j) => {
		...
		return [
			...
			' ',
			chalk.yellow(renderDur(first.departure, last.arrival)),
			'   ',
			chalk.gray(
				renderTime(first.plannedDeparture, first.departureDelay, {onTimeWithColor: false})
				+ ' – '
				+ renderTime(last.plannedArrival, last.arrivalDelay, {onTimeWithColor: false})
			)
		].join('')
	}
